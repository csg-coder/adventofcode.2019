...
namespace console_day_7
{
    class Program2
    {
                    ...
                    for (int c = 0; c < 5; c++)
                        for (int d = 0; d < 5; d++)
                            for (int e = 0; e < 5; e++)
                            {
                                ...
                                var puzzlecopy = puzzle7.ToList<int>().ToArray();
                                var next = "0";
                                next = Process(puzzlecopy, a.ToString(), next);
                                //Console.WriteLine($"next={next}");
                                next = Process(puzzlecopy, b.ToString(), next);
                                //Console.WriteLine($"next={next}");
                                next = Process(puzzlecopy, c.ToString(), next);
                                //Console.WriteLine($"next={next}");
                                next = Process(puzzlecopy, d.ToString(), next);
                                //Console.WriteLine($"next={next}");
                                next = Process(puzzlecopy, e.ToString(), next);

                                var nexti = int.Parse(next);
                                if (max < nexti)
                                    max = nexti;
                                Console.WriteLine($"canditate={next};max={max}");
        ...
        static string Process(int[] puzzle, string firstinput, string secondinput)
        {
            var cnt = 0;
            var ret = "";
            var l = puzzle.Length;
            ...
            while (i < l)
            {
                ...
                else if (opcode == 3)
                {
                    ...
                    if (cnt == 0)
                        ...
                        throw new Exception("third input not expected..");
