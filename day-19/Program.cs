...
namespace day_19
{
    class day_19
    {
        static void Main(string[] args)
        {
            ...
            Assert.Equal(250020, Solve2(() => ReadBeam("sample-problem2.txt"), 10));
            Assert.Equal(9480761, Solve2(() => DetectBeam(program, 1100), 100));
        ...
        private static List<Tuple<int, int>> ReadBeam(string file)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                beam.Add(new Tuple<int, int>(start, end));
            }
            return beam;
        }

        //private static void WriteBeam(string outfile, List<Tuple<int, int>> beam)
        //{
        //    for(int i=0;i<beam.Count; i++)
        //    {
        //        var line = "";
                
        //            line += new string(' ', beam[i].Item1 + 1);
        //        line += new string('#', beam[i].Item2-beam[i].Item1 + 1);
        //        File.AppendAllText(outfile, line + "\r\n");
        //    }
        //}

        private static int Solve2(Func<List<Tuple<int, int>>> beamreader, int squaresize)
        {
            List<Tuple<int, int>> beam = beamreader();

            int ret = 0;

            for (int i = 5 + squaresize; i < beam.Count; i++)
            {
                Console.WriteLine($"{beam[i].Item1}-{beam[i].Item2}");
                var prevstart = beam[i - squaresize + 1].Item1;
                var prevend = beam[i - squaresize + 1].Item2;
                var start = beam[i].Item1;
                var end = beam[i].Item2;

                if (
                    end - start + 1 >= squaresize
                    &&
                    prevend - prevstart + 1 >= squaresize
                    )
                    if (prevstart <= start
                    &&
                    start <= prevend
                    && start + squaresize - 1 <= prevend)
                    {

                        ret = 10000 * start + (i - squaresize + 1);
                        break;
                    }
        ...
        private static List<Tuple<int, int>> DetectBeam(long[] program, int mapsize)
        {
            var beam = new List<Tuple<int, int>>();

            var l = 1;
            var c = 1;
            beam.Add(new Tuple<int, int>(0, 0));
            ...
            while (l < mapsize)
            {
                ...
                while (c < mapsize)
                {
                    var pulled = Computer.Process(program, new string[] { c.ToString(), l.ToString() }) == "1";
                    if (pulled)
                    {
                        if (!encountered)
                            start = c;
